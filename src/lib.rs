...
pub trait Lottery {
	...
	fn start_lottery(
		...
		let mut lotteries = self.get_all_lotteries_info();
		lotteries.push(lottery_name.clone());
		self.set_all_lotteries_info(lotteries);
	...
	fn update_after_buy_ticket(
		...
		let mut holder_loteries = self.get_holder_loteries(&caller);
		if !holder_loteries.contains(&lottery_name) {
			let lname = lottery_name.clone();
			holder_loteries.push(lname);
			self.set_holder_loteries(&caller, holder_loteries);
		}
	...
	fn clear_storage(&self, lottery_name: &BoxedBytes) {
		let info = self.get_lottery_info(lottery_name);
		let lottery = lottery_name.clone();

		for i in 0..info.current_ticket_number {
			let addr = self.get_ticket_holder(lottery_name, i);

			let mut holder_loteries = self.get_holder_loteries(&addr);
			if holder_loteries.contains(lottery_name) {
				let index = holder_loteries.iter().position(|x| *x == lottery).unwrap();
				holder_loteries.remove(index);
				self.set_holder_loteries(&addr, holder_loteries);
			}

			self.clear_ticket_holder(lottery_name, i);
			self.clear_number_of_entries_for_user(lottery_name, &addr);
		}
		let mut all_lotteries = self.get_all_lotteries_info();
		let index_all_lotteries = all_lotteries.iter().position(|x| *x == lottery).unwrap();
		all_lotteries.remove(index_all_lotteries);
		self.set_all_lotteries_info(all_lotteries);
	...
	#[storage_set("holderLoteries")]
	fn set_holder_loteries(&self, ticket_holder: &Address, lottery_name: Vec<BoxedBytes>);

	#[view(holderLoteries)]
	#[storage_get("holderLoteries")]
	fn get_holder_loteries(&self, ticket_holder: &Address) -> Vec<BoxedBytes>;
	...
	#[storage_set("allLotteries")]
	fn set_all_lotteries_info(&self, lottery_name: Vec<BoxedBytes>);

	#[view(allLotteries)]
	#[storage_get("allLotteries")]
	fn get_all_lotteries_info(&self) -> Vec<BoxedBytes>;
